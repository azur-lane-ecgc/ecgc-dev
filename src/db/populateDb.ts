...
import type { ShipData, AllShipData } from "./types"
...
const populateDb = async () => {
  ...
  const allShips: AllShipData[] = Object.values(shipData).map((ship) => ({
    ...
    rankings: {
      mfRankings:
        ship.fleetType === "main" ? mainFleetRankings[ship.ship] || null : null,
      vgRankings:
        ship.fleetType === "vg" ? vgFleetRankings[ship.ship] || null : null,
      ssRankings:
